...
//第三个参数是什么
chordModule::chordModule(const ip_port& _selfAddr,  const ip_port& _joinAddr, size_t (*_hash)(const std::string&))
                         ...
                         const ip_port& requester)
{
  ...
  if (targetHashVal>selfHashVal && targetHashVal<=nextHashVal)
  {
    ...
    else
    {
      ...
      Communicate comm(requester.first.c_str(),requester.second);
      comm.sendString(outputConfig.c_str());
    }
  }
  else
  {
    ...
    outputConfig = writer.write(msg);
    Communicate comm(contactAddr.first.c_str(),contactAddr.second);
    comm.sendString(outputConfig.c_str());
  }
